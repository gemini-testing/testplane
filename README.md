...
`RUNNER_BEFORE_END`       | Will be triggered after test execution but before free resources. If a handler returns a promise, resources will be free only after the promise is resolved.
`RUNNER_END`              | Will be triggered after test execution and free resources. If a handler returns a promise, tests will be executed only after the promise is resolved. The handler accepts a stats of tests execution.
