...
module.exports = class NewBrowser extends Browser {
    ...
    _getSessionOpts() {
        ...
        const options = Object.assign({ protocol: gridUri.protocol(), hostname: this._getGridHost(gridUri), port: gridUri.port() ? parseInt(gridUri.port(), 10) : DEFAULT_PORT, path: gridUri.path(), queryParams: this._getQueryParams(gridUri.query()), capabilities, automationProtocol: config.automationProtocol, logLevel: this._debug ? "trace" : "error", connectionRetryTimeout: config.sessionRequestTimeout || config.httpTimeout, connectionRetryCount: 0, baseUrl: config.baseUrl, waitforTimeout: config.waitTimeout, waitforInterval: config.waitInterval }, this._getSessionOptsFromConfig());
        return options;
    }
    ...
    _addHeadlessCapability(headless, capabilities) {
        ...
        const browserCapabilities = (_a = capabilities[capabilitySettings.capabilityName]) !== null && _a !== void 0 ? _a : {};
        capabilities[capabilitySettings.capabilityName] = Object.assign(Object.assign({}, browserCapabilities), { args: [...((_b = browserCapabilities.args) !== null && _b !== void 0 ? _b : []), ...capabilitySettings.args] });
        return capabilities;
    }
