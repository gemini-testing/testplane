...
'use strict';
var util = require('./util'), rect = require('./rect'), lib = require('./lib'), queryIgnoreAreas = require('./ignore-areas'), Rect = rect.Rect;
...
function prepareScreenshotUnsafe(areas, opts) {
    ...
    var viewportHeight = document.documentElement.clientHeight, viewportWidth = document.documentElement.clientWidth, documentHeight = document.documentElement.scrollHeight, documentWidth = document.documentElement.scrollWidth, viewPort = new Rect({
...
function getElementCaptureRect(element, opts) {
    var pseudo = [':before', ':after'], css = lib.getComputedStyle(element), clientRect = rect.getAbsoluteClientRect(element, opts.scrollElem);
...
function getExtRect(css, clientRect, allowViewportOverflow) {
    var shadows = parseBoxShadow(css.boxShadow), outline = parseInt(css.outlineWidth, 10);
    if (isNaN(outline)) {
        outline = 0;
    }
...
function parseBoxShadow(value) {
    value = value || '';
    var regex = /[-+]?\d*\.?\d+px/g, values = value.split(','), results = [], match;
...
function adjustRect(rect, shadows, outline, allowViewportOverflow) {
    var shadowRect = calculateShadowRect(rect, shadows, allowViewportOverflow), outlineRect = calculateOutlineRect(rect, outline, allowViewportOverflow);
    return shadowRect.merge(outlineRect);
}
function calculateOutlineRect(rect, outline, allowViewportOverflow) {
    var top = rect.top - outline, left = rect.left - outline;
    return new Rect({
...
function calculateShadowRect(rect, shadows, allowViewportOverflow) {
    var extent = calculateShadowExtent(shadows), left = rect.left + extent.left, top = rect.top + extent.top;
    return new Rect({
        left: allowViewportOverflow ? left : Math.max(0, left),
        top: allowViewportOverflow ? top : Math.max(0, top),
