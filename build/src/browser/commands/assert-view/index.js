...
module.exports.default = browser => {
    ...
    const assertView = async (state, selectors, opts) => {
        ...
        try {
            ...
            await currImgInst.save(currImg.path);
            return handleInvalidRefImage(currImg, refImg, state, { emitter }).catch(e => handleCaptureProcessorError(e));
        }
    ...
    const waitSelectorsForExist = async (browser, selectors) => {
        await Promise.map([].concat(selectors), async (selector) => {
            await browser
                ...
                .catch(() => {
                throw new Error(`element ("${selector}") still not existing after ${browser.options.waitforTimeout} ms`);
            });
        });
    };
