...
export declare namespace lastFailed {
    let only: boolean;
    let output: string;
    let input: string;
}
export declare namespace devServer {
    ...
    namespace readinessProbe {
        ...
        let isReady: null;
        namespace timeouts {
            let waitServerTimeout: number;
            let probeRequestTimeout: number;
            let probeRequestInterval: number;
        }
    }
}
export declare let passive: boolean;
export declare let configPaths: string[];
export { WEBDRIVER_PROTOCOL as automationProtocol, NODEJS_TEST_RUN_ENV as testRunEnv };
