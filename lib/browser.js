...
module.exports = inherit({
    ...
    _createSession: function() {
        var config = this._config,
            ...
            session = webdriverio.remote({
                ...
                screenshotOnReject: config.screenshotOnReject,
    ...
    _addMetaAccessCommands: function(session) {
        session.addCommand('setMeta', (key, value) => this._meta[key] = value);
        session.addCommand('getMeta', (key) => this._meta[key]);
    },

    _decorateUrlMethod: function(session) {
        const baseUrlFn = session.url.bind(session);

        session.addCommand('url', (uri) => {
            if (uri) {
                const url = this._config.baseUrl && uri[0] === '/'
                    ? this._config.baseUrl + uri
                    : uri;

                this._meta.url = URI(url).origin('').toString();
            }
