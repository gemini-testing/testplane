...
module.exports = (browser) => {
    ...
    const {tolerance, antialiasingTolerance, stopOnFirstFail} = config;
            ...
            .then((page) => {
                return screenShooter.capture(page)
                    ...
                    .then(() => {
                        ...
                        const compareOpts = {tolerance, antialiasingTolerance, canHaveCaret, pixelRatio, stopOnFirstFail};

                        return Image.compare(refPath, currPath, compareOpts)
                            .then(({equal, diffBounds}) => {
                                if (!equal) {
                                    const opts = {diffBounds, config};
                                    return handleImageDiff(currPath, refPath, state, opts).catch((e) => handleCaptureProcessorError(e));
                                }
