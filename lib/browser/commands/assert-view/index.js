...
module.exports = (browser) => {
    ...
    const {tolerance, antialiasingTolerance, compareOpts} = config;
    ...
    session.addCommand('assertView', async (state, selectors, opts = {}) => {
        ...
        const imageCompareOpts = {tolerance, antialiasingTolerance, canHaveCaret, pixelRatio, compareOpts};
        const {equal, diffBounds, metaInfo = {}} = await Image.compare(refImg.path, currImg.path, imageCompareOpts);
        Object.assign(refImg, metaInfo.refImg);

        if (!equal) {
            const opts = {diffBounds, config};
