...
module.exports = (browser) => {
    ...
    session.addCommand('assertView', (state, selectors, opts = {}) => {
        ...
        const assertViewResults = test.hermioneCtx.assertViewResults || AssertViewResults.create();
        test.hermioneCtx.assertViewResults = assertViewResults;

        const handleCaptureProcessorError = (e) => e instanceof BaseStateError ? assertViewResults.add(e) : Promise.reject(e);
                    ...
                    .then(() => {
                        ...
                        return Image.compare(refPath, currPath, compareOpts)
                            .then((isEqual) => {
                                ...
                                assertViewResults.add({stateName: state, refImagePath: refPath});
                            });
                    })
                    .then(() => assertViewResults.get());
            });
    });
};
