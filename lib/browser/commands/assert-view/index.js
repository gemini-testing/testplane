...
module.exports = (browser) => {
    ...
    session.addCommand('assertView', async function(state, selectors, opts = {}) {
        await Promise.map([].concat(selectors), async (selector) => {
            await this.$(selector)
                ...
                .catch(() => {
                    throw new AssertViewError(`element ("${selector}") still not existing after ${this.options.waitforTimeout} ms`);
                });
        }
        );
    ...
    session.addCommand('assertView', async function(state, opts = {}) {
        await this.waitForExist({timeoutMsg: 'custom timeout msg'})
            .catch(() => {
                throw new AssertViewError(`element ("${this.selector}") still not existing after ${this.options.waitforTimeout} ms`);
            });
