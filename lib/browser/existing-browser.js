...
module.exports = class ExistingBrowser extends Browser {
    ...
    async init(sessionId, calibrator) {
        ...
        await this._resetCursorPosition();
    ...
    async _resetCursorPosition() {
        const baseDeprecationWarnings = this._session.options.deprecationWarnings;
        this._session.options.deprecationWarnings = false;

        // TODO: use "actions" method when it will be implemented in all webdrivers
        await Promise.resolve(this._session.moveToObject('body', 0, 0))
            .finally(() => this._session.options.deprecationWarnings = baseDeprecationWarnings);
    }
