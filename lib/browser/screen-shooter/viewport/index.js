...
module.exports = class Viewport {
    ...
    constructor(page, image, opts) {
        ...
        this._scaledDocumentWidth = Math.ceil(page.documentWidth * this._pixelRatio);
        this._scaledDocumentHeight = Math.ceil(page.documentHeight * this._pixelRatio);

        this._viewport = this._limitOverallDimensions(this._scale(page.viewport));
        this._captureArea = this._limitOverallDimensions(this._scale(this._sanitize(page.captureArea)));
        this._ignoreAreas = page.ignoreAreas.map(area => this._limitOverallDimensions(this._scale(area)));
        this._image = image;
        this._opts = opts;
    ...
    async extendBy(scrollHeight, newImage) {
        const physicalScrollHeight = Math.ceil(scrollHeight * this._pixelRatio);
    ...
    getVerticalOverflow() {
        return Math.ceil((getAreaBottom(this._captureArea) - getAreaBottom(this._viewport)) / this._pixelRatio);
    }

    _scale(area, scaleFactor = this._pixelRatio) {
        return {
            left: Math.floor(area.left * scaleFactor),
            top: Math.floor(area.top * scaleFactor),
            width: Math.ceil(area.width * scaleFactor),
            height: Math.ceil(area.height * scaleFactor)
        };
    }

    _limitOverallDimensions(area = {}) {
        return {
            ...area,
            width: clamp(area.width, 0, this._scaledDocumentWidth),
            height: clamp(area.height, 0, this._scaledDocumentHeight)
        };
    }
