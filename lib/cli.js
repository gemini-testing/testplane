...
const runHermione = (commander) => {
    const configPath = commander.config || defaults.config;
    const cliOpts = _.set({}, 'system.mochaOpts.grep', commander.grep);
    const options = _.set({}, 'reporters', commander.reporter || defaults.reporters);

    const config = Config.create(configPath, cliOpts, {cli: true, env: true});

    validateEmptyBrowsers(config.browsers);

    logRunData(config);

    return Hermione.create(config, options).run(commander.args, commander.browser);
};
