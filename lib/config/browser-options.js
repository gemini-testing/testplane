...
function parseSaveHistory(value) {
...
function buildBrowserOptions(defaultFactory, extra) {
    ...
    return _.extend(extra, {
        ...
        saveHistory: option({
            defaultValue: defaultFactory('saveHistory'),
            parseEnv: parseSaveHistory,
            parseCli: parseSaveHistory,
            validate: (value) => {
                const availableValues = [false, 'onlyFailed', true];

                if (!availableValues.includes(value)) {
                    throw new Error(`"saveHistory" must be one of: ${availableValues.join(', ')}`);
                }
            }
        }),
