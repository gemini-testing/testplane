...
module.exports = class Viewport {
    ...
    async ignoreAreas(image, imageArea) {
        for (const area of this._ignoreAreas) {
            ...
            if (imageClearArea !== null) {
                await image.addClear(this._shiftArea(imageClearArea, -imageArea.left, -imageArea.top));
            }
        }
    ...
    async handleImage(image, area = {}) {
        ...
        await this.ignoreAreas(image, this._shiftArea(capturedArea, -area.left, -area.top));
    ...
    _transformToCaptureArea(area) {
        const intersectingArea = this._getIntersection(
            this._shiftArea(area, this._viewport.left, this._viewport.top + this._summaryHeight),
            this._shiftArea(this._captureArea, area.left, area.top)
        );

        if (!this._summaryHeight) {
            intersectingArea.height = Math.min(area.height, this._captureArea.height);
        }
    ...
    _transformToViewportOrigin(area) {
        return this._shiftArea(area, -this._viewport.left, -this._viewport.top - this._summaryHeight);
    }
};
