...
module.exports = class Hermione extends BaseHermione {
    ...
    run(source, options = {}) {
        const {browsers, reporters, grep, updateRefs, sets: cliSets} = options;
        ...
        _.extend(this._config.system.mochaOpts, {grep});

        RuntimeConfig.getInstance().extend({updateRefs});

        const getSourcesByBro = () => {
            return source instanceof TestCollection
                ? source.getEnabledTestsByBro()
                : sets.reveal(this._config.sets, {paths: source, browsers, sets: cliSets});
        };

        return this._init()
            .then(() => getSourcesByBro())
            .then((sourcesByBro) => this._runner.run(sourcesByBro))
            .then(() => !this.isFailed());
    }
    ...
    readTests(testPaths, browsers, options = {}) {
        ...
        return init
            ...
            .then((suiteTree) => TestCollection.create(suiteTree));
    }
