...
module.exports = class BaseReporter {
    ...
    _onRetry(test) {
        ...
        // test.retriesLeft may be negative if 'shouldRetry' handler defined.
        const retriesLeft = Math.max(0, test.retriesLeft);
        logger.log('Will be retried. Retries left: %s', chalk.yellow(retriesLeft));
    }
