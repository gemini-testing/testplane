...
module.exports = inherit({
    __constructor: function() {
        this._passed = this._failed = this._skipped = this._pending = this._broken = 0;
    },
    ...
    attachRunner: function(runner) {
        ...
        runner.on(RunnerEvents.TEST_SKIP, this._onTestSkip.bind(this));
    ...
    _onTestFail: function(test) {
        ...
        logger.log('Error: %s', chalk.red(test.error));
    },

    _onTestSkip: function(test) {
        this._skipped++;
    ...
    _onRunnerEnd: function() {
        var total = this._passed + this._failed + this._skipped + this._pending + this._broken;

        logger.log('Total: %s Passed: %s Failed: %s Broken: %s Skipped: %s Pending: %s',
            chalk.underline(total),
            chalk.green(this._passed),
            chalk.red(this._failed),
            chalk.red(this._broken),
    ...
    _onError: function(error) {
        logger.log('Error: %s', chalk.red(error));
    },
