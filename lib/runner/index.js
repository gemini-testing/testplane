...
module.exports = class MainRunner extends BaseRunner {
    ...
    run(options) {
        ...
        return this.emitAndWait(RunnerEvents.RUNNER_START, this)
            .then(() => this._createWorkerFarm(options))
            .then((w) => workers = w)
    ...
    _createWorkerFarm(options) {
        ...
        const workers = workerFarm(params, workerFilepath, {init: {broadcast: true}, runTest: {}});

        const defer = q.defer();
        workers.init(this._config, options, (err, results) => {
            if (err) {
                defer.reject(err);
            } else {
                defer.resolve();
