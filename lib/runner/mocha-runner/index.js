...
var MochaRunner = inherit(Runner, {
    ...
    run: function(suitePaths, filterFn) {
        return _(suitePaths)
            .map(_.bind(this._runFile, this, _, filterFn))
            .thru(utils.waitForResults)
            .value();
    },

    _runFile: function(suiteFile, filterFn) {
        return this._createMocha(suiteFile, filterFn)
            .run();
    },

    _createMocha: function(suiteFile, filterFn) {
        var mocha = new MochaAdapter(this._config.mochaOpts);
