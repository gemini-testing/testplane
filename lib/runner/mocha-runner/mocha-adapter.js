...
module.exports = class MochaAdapter {
    ...
    _injectRunnableSpy() {
        ...
        this._addEventHandler(
            ...
            this._overrideRunnableFn((runnable) => {
                const _this = this;
    ...
    _injectBeforeHookErrorHandling() {
        this._addEventHandler('beforeAll', this._overrideRunnableFn((runnable) => {
            const baseFn = runnable.fn;

            runnable.fn = function() {
                return q(baseFn).apply(this, arguments)
                    .catch((error) => {
                        runnable.parent.failed = error;
                        runnable.parent.eachTest((test) => test.failed = error);
                    });
            };
        }));
    }
    ...
    _injectBeforeEachHookErrorHandling() {
        this._addEventHandler('beforeEach', this._overrideRunnableFn((runnable) => {
            const baseFn = runnable.fn;

            runnable.fn = function() {
                return q(baseFn).apply(this, arguments)
                    .catch((error) => runnable.ctx.currentTest.failed = error);
            };
        }));
    }

    _injectPretestFailVerification() {
        this._addEventHandler('test', this._overrideRunnableFn((runnable) => {
            const baseFn = runnable.fn;

            runnable.fn = function() {
                return runnable.failed
                    ? q.reject(runnable.failed)
    ...
    _overrideRunnableFn(overrideFn) {
        return (runnable) => runnable.fn ? overrideFn(runnable) : undefined;
    }
