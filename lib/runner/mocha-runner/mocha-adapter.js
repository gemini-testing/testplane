...
module.exports = class MochaAdapter extends EventEmitter {
    ...
    _passthroughFileEvents() {
        ...
        const emit_ = (event, file) => this.emit(event, {
            file,
        ...
        this.suite.on('pre-require', (ctx, file) => {
            ...
            emit_(RunnerEvents.BEFORE_FILE_READ, file);
        });
    ...
    parse() {
        const tests = [];
        this._mocha.reporter(function(runner) {
            runner.on('test end', (test) => tests.push(test));
        });
        this.suite.eachTest((test) => test.fn = () => {});

        return new Promise((resolve) => this._mocha.run(() => resolve(tests)));
    }
