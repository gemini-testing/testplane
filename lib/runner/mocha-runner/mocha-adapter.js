...
module.exports = class MochaAdapter {
    ...
    applyGrep(pattern) {
        ...
        const filterTree = (node) => {
            node.tests = node.tests.filter((test) => pattern.test(test.fullTitle()));
            node.suites = _(node.suites)
                .map((suite) => {
                    if (pattern.test(suite.fullTitle())) {
                        return suite;
                    }

                    filterTree(suite);

                    if (_.isEmpty(suite.suites) && _.isEmpty(suite.tests)) {
                        return null;
                    }
                    return suite;
                })
