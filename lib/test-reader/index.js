...
module.exports = class TestReader extends EventEmitter {
    ...
    async read({paths, browsers, ignore, sets, grep} = {}) {
        ...
        const specs = {};
        for (let browserId in filesByBro) {
            specs[browserId] = await this._parseBrowserTests(filesByBro[browserId], browserId, grep);
        }

        return specs;
    }

    async _parseBrowserTests(files, browserId, grep) {
