...
module.exports = class ExecutionThread {
    ...
    _call(runnable) {
        ...
        return fnPromise.finally(() => {
            const isRejected = fnPromise.isRejected();
            const hasAssertErrors = this._hermioneCtx.assertViewResults && this._hermioneCtx.assertViewResults.hasFails();

            if (isRejected || hasAssertErrors) {
                return this._screenshooter.extendWithPageScreenshot(this._hermioneCtx, {test: this._ctx.currentTest});
            }
        });
    }
