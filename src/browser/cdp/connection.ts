...
export class CDPConnection {
    ...
    private async tryToEstablishWsConnection(endpoint: string): Promise<WebSocket | Error> {
    ...
    private async getWsConnection(): Promise<WebSocket> {
        ...
        if (this._wsConnectionStatus === WsConnectionStatus.CONNECTED && ws && ws.readyState !== ws.OPEN) {
            this.closeWsConnection("CDP connection was in invalid state", WsConnectionStatus.DISCONNECTED);
        }
        ...
        this._wsConnectionPromise = (async (): Promise<WebSocket> => {
            try {
                for (let retriesLeft = CDP_CONNECTION_RETRIES; retriesLeft >= 0; retriesLeft--) {
                    const result = await this.tryToEstablishWsConnection(this._cdpWsEndpoint);
                    ...
                    if (result instanceof WebSocket) {
                        ...
                        this.pingHealthCheckStart();

                        const onPing = (): void => result.pong();
                        const onMessage = (data: RawData): void => this.onMessage(data);
    ...
    async request<T>(
        ...
        for (let retriesLeft = CDP_REQUEST_RETRIES; retriesLeft >= 0; retriesLeft--) {
            result = (await this.tryToSendRequest(method, { params, sessionId })) as T | Error;
    ...
    private closeWsConnection(
        ...
        this.abortPendingRequests(`Request was aborted because ${sessionAbortMessage}`);
        this.pingHealthCheckStop();
    ...
    close(): void {
        this.closeWsConnection("Connection was closed manually", WsConnectionStatus.CLOSED);
    }

    private pingHealthCheckStop(): void {
    ...
    private isWebSocketActive(ws: WebSocket): boolean {
        return Boolean(ws.readyState !== ws.CLOSED && ws.readyState !== ws.CLOSING && ws === this._wsConnection);
    }

    private pingHealthCheckStart(): void {
        this.pingHealthCheckStop();

        const ws = this._wsConnection;

        if (!ws || !this.isWebSocketActive(ws)) {
            return;
        }
        ...
        const onPong = (this._onPong = (): void => {
            if (isWaitingForPong && this.isWebSocketActive(ws)) {
