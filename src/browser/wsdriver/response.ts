...
export const parseWsDriverIncomingMessage = async (
    ...
    try {
        const decompressedBody: Error | Buffer = await getDecompressed(rawBody, compressionType).catch(err => err);

        if (decompressedBody instanceof Error) {
        ...
        body = new Error("Couldn't parse incoming JSON payload", { cause });
    }
