...
describe('config browser-options', () => {
    ...
    describe('testsPerSession', () => {
        describe('should throw an error if the option', () => {
            it('is not a number', () => {
                const readConfig = {
                    browsers: {
            ...
            it('is a float number', () => {
                const readConfig = {
                    browsers: {
                        b1: mkBrowser_({testsPerSession: 1.5})
                    }
                };
        ...
        it('should not throw an error if the option is set to infinity', () => {
            const readConfig = {
                browsers: {
                    b1: mkBrowser_({testsPerSession: Infinity})
                }
            };

            Config.read.returns(readConfig);

            assert.doesNotThrow(() => createConfig(), Error, `"testsPerSession" must be a positive integer`);
        });

        it('should set the option to all browsers', () => {
            const readConfig = {
                testsPerSession: 777,
                browsers: {
                    b1: mkBrowser_(),
            ...
            assert.equal(config.browsers.b1.testsPerSession, 777);
            assert.equal(config.browsers.b2.testsPerSession, 777);
        });

        it('should override the option for the specified browsers', () => {
            const readConfig = {
                testsPerSession: 777,
                browsers: {
                    b1: mkBrowser_(),
                    b2: mkBrowser_({testsPerSession: 555})
                }
            };
            ...
            assert.equal(config.browsers.b1.testsPerSession, 777);
            assert.equal(config.browsers.b2.testsPerSession, 555);
        });
    });
