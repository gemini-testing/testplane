...
describe("browser-installer/firefox/browser", () => {
    ...
    describe("installFirefox", () => {
        it("should try to resolve browser path locally by default", async () => {
            ...
            const binaryPath = await installFirefox("115", { browserDownloadMirrors });
        ...
        it("should download browser from mirror", async () => {
            ...
            assert.calledWithExactly(
                ...
                `downloading firefox@stable_115.0 from mirror ${browserDownloadMirrors.firefox}`,
            );
        });
        ...
        it("should report a configured mirror error when the browser artifact is unavailable", async () => {
            ...
            assert.equal(
                error.message,
                `Couldn't download browser artifact from the configured mirror: ${browserDownloadMirrors.firefox}`,
            );
            assert.neverCalledWith(browserInstallerDebugStub, sinon.match(/from mirror/));
        });
