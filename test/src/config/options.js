        ...
        ["chrome", "chromium", "firefox"].forEach(browserName => {
            it(`should reject a non-absolute ${browserName} mirror URL`, () => {
                assert.throws(
                    ...
                    `"browserDownloadMirrors.${browserName}" must be a valid URL, got mirror.example/browser`,
                );
            });
        });
        ...
        [
            {
                ...
                error: '"browserDownloadMirrors.chrome" must be an absolute http: or https:, got ftp',
            },
            {
                name: "a username",
        ...
        it("should reject empty uppercase environment overrides for every mirror", () => {
            [
                ...
                assert.throws(
                    () =>
                        parse_({
                            options: { browserDownloadMirrors: { [browserName]: "https://config.example/browser" } },
                            env: { [envName]: "" },
                        }),
